...
class StockEnvPlayer(gym.Env):
    ...
    def __init__(
        ...
        self.action_space = spaces.Box(
            low=-self.numSecurity,
            high=self.numSecurity,
            shape=(self.numSecurity,),
            dtype=np.int8,
        )
        ...
        with open(logfile, "a+") as f:
            ...
            others = [
                "asset" + str(i) + "_" + col
                for i in range(self.numSecurity)
                for col in self.TA_columns
            ]
            column = (
                "model, incTA?, step, date, cash, portfolio, reward, total_commission, buy_amt, buy_commission, sell_amt, sell_commission,"
                + ",".join(ap)
                + ","
                + ",".join(aq)
                + ","
    ...
    def _update_df(
        self, df_new
    ):  # Now can update dataframe via this funtion and can store it
        self.df = df
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            portfolio_value = self.state[0] + sum(
                np.array(self.price) * np.array(self.qty)
            )
    ...
    def render(self, mode="human"):
        line = np.array(
            [
                ...
                self.addTA,
                self.day,
                str(self.dates[self.day]),
                str(self.value[0]),
                str(self.portfolio_value[-1]),
                self.reward,
                self.commission_paid[self.day - 1],
                self.transaction["buy_amt"],
                self.transaction["buy_commission"],
                self.transaction["sell_amt"],
                self.transaction["sell_commission"],
            ]
        )
